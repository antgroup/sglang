...
`quality` is a request-scoped sampling parameter with two levels:
...
- `"high"`: the accelerated Cache-DiT path. On CUDA SM90 and SM100 with
  `ring_degree=1` and graph compilation disabled, it also enables SubBlock
  sparse attention while keeping attention dense for the first ten logical
  denoising steps. Other CUDA architectures keep dense attention and still use
  Cache-DiT. Output is not bit-identical to the reference.

One resident server serves both levels; a `quality: "high"` request mounts
its Cache-DiT policy and eligible SubBlock backend at the batch boundary. A
later `quality: "lossless"` request restores the server's dense attention
backend and removes the Cache-DiT hooks before denoising.

Start a server once. This example is the measured 4×H200 profile:
...
The accelerated path. Use it when you can trade bit-exactness for latency while
keeping output close to the same-seed lossless trajectory.
...
The published Cache-DiT-only trade-off, measured before the SubBlock path was
added to `quality: "high"`, is:
...
The combined Cache-DiT + SubBlock path requires a new same-workload quality and
latency measurement. Do not use the Cache-DiT-only row above as evidence for
the combined path.
...
`quality: "high"` currently accepts only the exact workload above: T2VA at
1344×768, 124 frames, 24 fps, 50 steps, video flow shift 12, and audio flow
shift 3. CUDA GPU product names and parallel topology are not allowlisted.
Cache-DiT support is validated independently, while SubBlock is enabled only
on SM90 or SM100 when its execution-mode requirements are met. The measurements
above cover 4×H200 Cache-DiT only; qualify other deployments and the combined
path separately. Offline generation uses the same level name, for example
`sglang generate --quality high`.
...
combined with FSDP inference, DiT layerwise offload, or breakable CUDA graphs.
Those combinations reject a `quality: "high"` request unless the request sets
`enable_cache_dit: false`. MiniMax-H3 Cache-DiT is not enabled on MPS. SubBlock
falls back to the server's dense attention backend on unsupported SM versions,
Ring Attention, `torch.compile`, or breakable CUDA graphs. Tune the cache
thresholds only after comparing both video and audio quality on the target task
profile. A real B200 Cache-DiT request has completed, but only the 4×H200
Cache-DiT-only profile above has published quality and latency measurements.
</Warning>
...
remain BF16. This is an approximate serve-time choice. It can be combined with
`quality="high"`, but that combination is outside the measured 4×H200 profile.
