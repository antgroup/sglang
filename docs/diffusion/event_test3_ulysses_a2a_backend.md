...
模型目标：LingBot World fast diffusers（非量化）
更新时间：2026-07-23
...
当前实现会拒绝 `--ulysses-a2a-qkv-overlap=auto|on`，避免配置看似生效但实际
仍走串行 Q/K/V。生产模型必须使用不带 `compressed-tensors`/FP8
`quantization_config` 的 BF16/FP16 checkpoint；本分支不会把量化权重反量化。
...
能力检查发生在共享内存分配前，并同时检查所选 TK/legacy launch op 以及
`meta_size`、`init_ulysses_a2a`、`dispose_ulysses_a2a` 的 Python wrapper 和
Torch op。signal/output allocation 一旦开始，后续异常属于 post-commit
failure，禁止回退 NCCL。
...
- variable split：先用共享 `seq_lens` 校验本 rank 的 local sequence dimension，
  再归一化该 dimension 做 descriptor 比较；合法的不等长固定排除 P2P，
  `seq_lens` 或其他语义不一致则 fail fast；
...
本版本中 overlap 仅接受 `off`；`auto` 和 `on` 是保留值，启动时会 fail
closed。
...
- 目标文件 `compileall`；
- 目标文件 Ruff。
- 量化提交的最终 tree 差异已完全撤销，quantization registry 和测试恢复到
  `event_test3`；
- 新增 CPU 回归覆盖配置优先级、未实现配置拒绝、TK/legacy symbol 预检、
  variable-split descriptor 归一化、post-commit 错误分类和 CUDA IPC close。

`scripts/ulysses_a2a/run_static_checks.sh` 的 compile/Ruff 阶段通过；运行 smoke
import 时，本机 Python 因缺少既有依赖 `cloudpickle` 停止。导入已经越过本次
撤销的 compressed-tensors 循环依赖位置。这里不为仓库临时安装依赖；应在完整
SGLang 生产镜像中重跑 smoke/registered tests。本机没有 CUDA/NVLink，因此
也不能把本地结果当作多 rank、模型或性能验收。
...
- 模型沿用 `event_test3` 的非量化 BF16/FP16 加载路径，本分支不引入新的量化
  loader 或启动期量化依赖；
