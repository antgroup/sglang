...
class TestUlyssesA2AArgs(unittest.TestCase):
    ENV_KEYS = (
    ...
    def _resolve(self, **overrides):
        ...
        args.ulysses_a2a_backend = overrides.get("backend")
        args.ulysses_a2a_transfer = overrides.get("transfer")
        args.ulysses_a2a_qkv_overlap = overrides.get("overlap")
        args.ulysses_a2a_legacy_prefer_p2p = False
        args._adjust_ulysses_a2a()
        args._validate_ulysses_a2a()
        return args

    def test_default_and_new_environment_resolution(self):
        with patch.dict(os.environ, {}, clear=True):
            args = self._resolve()
            self.assertEqual(args.ulysses_a2a_backend, "nccl")
            self.assertEqual(args.ulysses_a2a_transfer, "auto")
            self.assertEqual(args.ulysses_a2a_qkv_overlap, "off")

        with patch.dict(
            os.environ,
            {
                "SGLANG_ULYSSES_A2A_BACKEND": "fast_ulysses",
                "SGLANG_ULYSSES_A2A_TRANSFER": "ce",
                "SGLANG_ULYSSES_A2A_ASYNC_QKV": "on",
            },
            clear=True,
        ):
            args = self._resolve()
            self.assertEqual(args.ulysses_a2a_backend, "fast_ulysses")
            self.assertEqual(args.ulysses_a2a_transfer, "ce")
            self.assertEqual(args.ulysses_a2a_qkv_overlap, "on")

    def test_explicit_cli_value_wins_over_new_and_legacy_environment(self):
        with patch.dict(
            os.environ,
            {
                "SGLANG_ULYSSES_A2A_BACKEND": "fast_ulysses",
            ...
            args = self._resolve(backend="nccl")
            self.assertEqual(args.ulysses_a2a_backend, "nccl")

    def test_conflicting_legacy_flags_are_rejected(self):
        with patch.dict(
            os.environ,
            {
                "SGLANG_ENABLE_ULYSSES_P2P_A2A": "1",
    ...
    def test_transfer_engine_is_rejected_for_non_fast_backend(self):
        with patch.dict(
            ...
            with self.assertRaisesRegex(ValueError, "only valid"):
                self._resolve(backend="nccl", transfer="sm")
