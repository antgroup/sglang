...
class GPUWorker(GPUWorkerPostTrainingMixin):
        ...
        *,
        ...
        for item in log_reqs:
            if item.metrics is not None:
                item.metrics.session_id = (
                    getattr(item, "realtime_session_id", None) or session_id
                )
        with log_session_context(session_id):
            return self._execute_forward_common_impl(
                req,
    ...
    def _execute_forward_common_impl(
        self,
        req: Req,
        *,
        forward_fn: Callable[[], Req | OutputBatch],
