...
template <typename T, int64_t kElementSize, uint32_t kUnroll, uint32_t kBlockQuota, uint32_t kBlockSize>
SGL_HICACHE_KERNEL void hicache_transfer_per_layer_pf_lf(const __grid_constant__ HicachePfKernelParams params) {
  using namespace device;
  static_assert(kBlockSize % kWarpThreads == 0);
  static_assert(kWarpThreads % kUnroll == 0);
...
template <typename T, int64_t kElementSize, uint32_t kUnroll, uint32_t kBlockQuota, uint32_t kBlockSize>
SGL_HICACHE_KERNEL void hicache_transfer_all_layer_lf_pf(const __grid_constant__ HicachePfKernelParams params) {
  using namespace device;
  using src_ptr_t = const void*;
  using dst_ptr_t = void*;
  ...
  for (uint32_t i = work_id; i < length; i += kNumWorkers) {
    ...
    for (std::size_t layer = 0; layer < num_layers; ++layer) {
      ...
      // Page first: base + page_id * page_dim + layer_id * item_size_bytes
      const auto dst_k =
          pointer::offset(static_cast<dst_ptr_t>(k_ptr_dst), pos_dst * dst_layout_dim + layer * kv_cache_dst_stride);
      const auto src_v = pointer::offset(v_cache_src, pos_src * kv_cache_src_stride);
      const auto dst_v =
          pointer::offset(static_cast<dst_ptr_t>(v_ptr_dst), pos_dst * dst_layout_dim + layer * kv_cache_dst_stride);
      const auto vec_k = load_vec<kElementSize, kNumThreads>(src_k);
      const auto vec_v = load_vec<kElementSize, kNumThreads>(src_v);
      store_vec<kElementSize, kNumThreads>(dst_k, vec_k);
      store_vec<kElementSize, kNumThreads>(dst_v, vec_v);
...
struct HiCacheKernel {
  ...
  template <typename T>
  static constexpr auto kernel_pf_lf =
      hicache_transfer_per_layer_pf_lf<T, kElementSize, kUnroll, kBlockQuota, kBlockSize>;
  template <typename T>
  static constexpr auto kernel_lf_pf =
      hicache_transfer_all_layer_lf_pf<T, kElementSize, kUnroll, kBlockQuota, kBlockSize>;
  ...
  static void run_pf_lf(
    ...
    const auto kernel = use_int32 ? kernel_pf_lf<int32_t> : kernel_pf_lf<int64_t>;
    LaunchKernel(num_blocks, kBlockSize, device)(kernel, params);
  }
  ...
  static void run_lf_pf(
    ...
    const auto kernel = use_int32 ? kernel_lf_pf<int32_t> : kernel_lf_pf<int64_t>;
    LaunchKernel(num_blocks, kBlockSize, device)(kernel, params);
  }
};
